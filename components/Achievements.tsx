...
const achievements = [
  ...
  {
    icon: "📊",
    title: "97.03 Percentile — Naukri Campus Young Turks 2025",
    org: "Certificate of Merit · India's Largest Campus Skill Contest",
    ...
    <section id="achievements" className="section-container relative">
      ...
      >
        <motion.div variants={fadeUp} className="mb-12">
          ...
          <h2 className="text-4xl md:text-5xl font-bold text-white mb-4">Achievements & Awards</h2>
            ...
            <motion.div key={i} variants={scaleIn}>
              <TiltCard className="h-full">
                ...
                >
                  ...
                  <p className="text-[#7F5AF0] text-sm mb-3 font-medium tracking-wide">{item.org}</p>
                  <p className="text-white/60 text-sm leading-relaxed mb-4 flex-grow">{item.desc}</p>
                  <p className="text-white/40 text-xs font-semibold tracking-wider uppercase">{item.date}</p>
                </div>
              </TiltCard>
            </motion.div>
          ))}
        ...
        <motion.div variants={fadeUp} className="mt-20">
          ...
          <div className="glass p-8 rounded-2xl hover:border-[#7F5AF0]/40 transition-colors duration-500 hover:shadow-[0_12px_40px_rgba(127,90,240,0.15)]">
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
              <div>
                <h4 className="text-xl font-semibold mb-2 text-white">B.Tech in Computer Science and Engineering</h4>
                <p className="text-[#7F5AF0] mb-2 font-medium">Guru Nanak Institutions Technical Campus</p>
                <p className="text-white/50 text-sm">Hyderabad, India</p>
              </div>
              <div className="md:text-right">
                <p className="text-white/50 font-medium">Expected July 2026</p>
              </div>
            </div>
          </div>
        </motion.div>
